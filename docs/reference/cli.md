...
Complete reference for the published `@kenanlian/opencode-agent-orchestrator` CLI.
...
```bash
# Display help
bunx @kenanlian/opencode-agent-orchestrator

# Or with npx
npx @kenanlian/opencode-agent-orchestrator
```
...
2. **Plugin Registration**: Registers `@kenanlian/opencode-agent-orchestrator` in OpenCode settings, or migrates legacy `oh-my-opencode` once
3. **Configuration File Creation**: Writes the generated config to `opencode-agent-orchestrator.json` in the active OpenCode config directory
...
- Legacy plugin entry references in `opencode.json` (warns when `oh-my-opencode` is still used instead of `@kenanlian/opencode-agent-orchestrator`)
...
```
opencode-agent-orchestrator doctor
...
Config
  ✓ opencode-agent-orchestrator.jsonc is valid
...
```bash
bunx @kenanlian/opencode-agent-orchestrator run <message>
```
...
```bash
bunx @kenanlian/opencode-agent-orchestrator get-local-version
```
...
```bash
bunx @kenanlian/opencode-agent-orchestrator version
```
...
bunx @kenanlian/opencode-agent-orchestrator mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes read write

# Remove stored OAuth tokens
bunx @kenanlian/opencode-agent-orchestrator mcp oauth logout <server-name> --server-url https://api.example.com

# Check OAuth token status
bunx @kenanlian/opencode-agent-orchestrator mcp oauth status [server-name]
```
...
1. **Project Level**: `.opencode/opencode-agent-orchestrator.jsonc` or `.opencode/opencode-agent-orchestrator.json`
2. **User Level**: `~/.config/opencode/opencode-agent-orchestrator.jsonc` or `~/.config/opencode/opencode-agent-orchestrator.json`

**Migration Note**: Legacy basename `oh-my-opencode.*` is migration input only. Canonical basename is `opencode-agent-orchestrator.*`.
...
bunx @kenanlian/opencode-agent-orchestrator doctor --verbose

# Show compact system dashboard
bunx @kenanlian/opencode-agent-orchestrator doctor --status

# JSON output for scripting
bunx @kenanlian/opencode-agent-orchestrator doctor --json
```

### "Using legacy package name" Warning

The doctor warns if it finds the legacy plugin entry `oh-my-opencode` in `opencode.json`. Update the plugin array to the canonical `@kenanlian/opencode-agent-orchestrator` entry:

```bash
# Replace the legacy plugin entry in user config
jq '.plugin = (.plugin // [] | map(if . == "oh-my-opencode" then "@kenanlian/opencode-agent-orchestrator" else . end))' \
...
| Option            | Description                                         |
| ----------------- | --------------------------------------------------- |
| `-d, --directory` | Working directory to read opencode-agent-orchestrator config from |
...
bunx @kenanlian/opencode-agent-orchestrator doctor --json > doctor-report.json
```
