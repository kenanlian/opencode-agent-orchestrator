...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[opencode-agent-orchestrator] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[opencode-agent-orchestrator] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[opencode-agent-orchestrator] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[opencode-agent-orchestrator] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
