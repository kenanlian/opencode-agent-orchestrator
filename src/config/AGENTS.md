...
24 schema files composing `OpenCodeAgentOrchestratorConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── opencode-agent-orchestrator-config.ts    # ROOT: OpenCodeAgentOrchestratorConfigSchema (composes all below)
...
2. Add field to `opencode-agent-orchestrator-config.ts` root schema
