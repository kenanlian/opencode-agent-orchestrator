...
/**
 * Get the cache directory for opencode-agent-orchestrator binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "opencode-agent-orchestrator", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[opencode-agent-orchestrator] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[opencode-agent-orchestrator] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[opencode-agent-orchestrator] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[opencode-agent-orchestrator] Comment checking disabled.`)
    return null
  }
}
