...
describe("migrateLegacyPluginEntry", () => {
  ...
  describe("#given opencode.json contains oh-my-opencode plugin entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces oh-my-opencode with @kenanlian/opencode-agent-orchestrator", async () => {
        ...
        expect(content).toContain("@kenanlian/opencode-agent-orchestrator@latest")
  ...
  describe("#given opencode.json contains bare oh-my-opencode entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces with @kenanlian/opencode-agent-orchestrator", async () => {
        const configPath = join(testDir, "opencode.json")
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain('"@kenanlian/opencode-agent-orchestrator"')
        expect(content).not.toContain("oh-my-opencode")
      })
    })
  })
    ...
    describe("#when migrating the config", () => {
      it("#then keeps the original config untouched and writes the migrated content to a sibling temp file", async () => {
        ...
        try {
          ...
          expect(readFileSync(tempPath, "utf-8")).toContain("@kenanlian/opencode-agent-orchestrator@latest")
  ...
  describe("#given opencode.json contains pinned oh-my-opencode version", () => {
    describe("#when migrating the config", () => {
      it("#then preserves the version pin", async () => {
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain("@kenanlian/opencode-agent-orchestrator@3.11.0")
      })
    })
  })

  describe("#given opencode.json already uses @kenanlian/opencode-agent-orchestrator", () => {
    describe("#when checking for migration", () => {
      it("#then returns false and does not modify the file", async () => {
        const configPath = join(testDir, "opencode.json")
        const original = JSON.stringify({ plugin: ["@kenanlian/opencode-agent-orchestrator@latest"] }, null, 2)
  ...
  describe("#given plugin entries contain both canonical and legacy values", () => {
    describe("#when migrating the config", () => {
      it("#then removes the legacy entry instead of duplicating the canonical one", async () => {
        const configPath = join(testDir, "opencode.json")
        writeFileSync(configPath, JSON.stringify({ plugin: ["@kenanlian/opencode-agent-orchestrator", "oh-my-opencode"] }, null, 2))
        const { migrateLegacyPluginEntry } = await importFreshMigrationModule()
        ...
        expect(saved.plugin).toEqual(["@kenanlian/opencode-agent-orchestrator"])
      })
    })
  })
...
`,
    ...
    "@kenanlian/opencode-agent-orchestrator@latest"
  ]`)
      })
    })
  })
