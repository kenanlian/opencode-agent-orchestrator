...
describe("migrateLegacyConfigFile", () => {
  ...
  describe("#given oh-my-opencode.jsonc exists but opencode-agent-orchestrator.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes opencode-agent-orchestrator.jsonc and renames the legacy file to a backup", () => {
        ...
        expect(existsSync(join(testDir, "opencode-agent-orchestrator.jsonc"))).toBe(true)
        expect(existsSync(legacyPath)).toBe(false)
        expect(existsSync(backupPath)).toBe(true)
        expect(readFileSync(join(testDir, "opencode-agent-orchestrator.jsonc"), "utf-8")).toBe('{ "agents": {} }')
        expect(readFileSync(backupPath, "utf-8")).toBe('{ "agents": {} }')
      })
    })
  })

  describe("#given oh-my-opencode.json exists but opencode-agent-orchestrator.json does not", () => {
    describe("#when migrating the config file", () => {
      it("#then copies to opencode-agent-orchestrator.json", () => {
        ...
        expect(result).toBe(true)
        expect(existsSync(join(testDir, "opencode-agent-orchestrator.json"))).toBe(true)
      })
    })
  })

  describe("#given opencode-agent-orchestrator.jsonc already exists", () => {
    describe("#when attempting migration", () => {
      it("#then returns false and does not overwrite", () => {
        const legacyPath = join(testDir, "oh-my-opencode.jsonc")
        const canonicalPath = join(testDir, "opencode-agent-orchestrator.jsonc")
