...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `@kenanlian/opencode-agent-orchestrator` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `opencode-agent-orchestrator.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `@kenanlian/opencode-agent-orchestrator@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/opencode-agent-orchestrator.jsonc` |
