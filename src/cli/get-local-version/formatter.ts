...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
  lines.push(color.bold(color.white("opencode-agent-orchestrator Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && bun update @kenanlian/opencode-agent-orchestrator")}`)
      break
    case "local-dev":
