...
describe("OpenClaw Dispatcher", () => {
  ...
  test("resolveCommandTimeoutMs reads OPENCODE_AGENT_ORCHESTRATOR env fallback", () => {
    const original = process.env.OPENCODE_AGENT_ORCHESTRATOR_OPENCLAW_COMMAND_TIMEOUT_MS
    process.env.OPENCODE_AGENT_ORCHESTRATOR_OPENCLAW_COMMAND_TIMEOUT_MS = "4321"
    ...
    try {
      ...
      if (original === undefined) delete process.env.OPENCODE_AGENT_ORCHESTRATOR_OPENCLAW_COMMAND_TIMEOUT_MS
      else process.env.OPENCODE_AGENT_ORCHESTRATOR_OPENCLAW_COMMAND_TIMEOUT_MS = original
    }
  })
