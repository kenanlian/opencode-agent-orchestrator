 ...
 * @returns {string} Package name like "@kenanlian/opencode-agent-orchestrator-darwin-arm64"
 * @throws {Error} If libc cannot be detected on Linux
 */
export function getPlatformPackage({ platform, arch, libcFamily, packageBaseName = "@kenanlian/opencode-agent-orchestrator" }) {
  let suffix = "";
  if (platform === "linux") {
...
export function getPlatformPackageCandidates({ platform, arch, libcFamily, preferBaseline = false, packageBaseName = "@kenanlian/opencode-agent-orchestrator" }) {
...
/** @param {{ platform: string, arch: string, libcFamily?: string | null, packageBaseName?: string }} options */
function getBaselinePlatformPackage({ platform, arch, libcFamily, packageBaseName = "@kenanlian/opencode-agent-orchestrator" }) {
  if (arch !== "x64") {
    return null;
  }
 ...
 * @returns {string} Relative path like "@kenanlian/opencode-agent-orchestrator-darwin-arm64/bin/opencode-agent-orchestrator"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/opencode-agent-orchestrator${ext}`;
}
