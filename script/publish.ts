...
const PACKAGE_NAME = "@kenanlian/opencode-agent-orchestrator"
...
console.log("=== Publishing @kenanlian/opencode-agent-orchestrator (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `@kenanlian/opencode-agent-orchestrator-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/kenanlian/opencode-agent-orchestrator/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/opencode-agent-orchestrator.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
